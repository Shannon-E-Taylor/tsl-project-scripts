...
I had hoped for a more convincing result, given this was obtained by excluding data. But the large effect size is reassuring, and the small sample size does give a low statistical power.  
...
- we can detect a (barely) significant difference in VM width in *tsl-* vs control wasps, when data from the termini are excluded. 
...
The termini of the *tsl-* oocytes do look different to the wild types. Therefore I wonder if it's worth looking at those further, as we are investigating the terminal patterning system. 
